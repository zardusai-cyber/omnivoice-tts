"""
OmniVoice INT4 Quantized API Server (XPU + TorchAO)
...
MODEL_INT4 = BASE_DIR / "OmniVoice_INT4"
VOICES_DIR = BASE_DIR / "voices"

app = FastAPI(title="OmniVoice TTS Server (INT4)", version="1.0.0")


def apply_quantization(model):
    from torchao.quantization import quantize_, Int4WeightOnlyConfig

    print("[2/5] Applying INT4 weight-only quantization...")
...
print("=" * 60)
print("OmniVoice TTS API Server (INT4 Quantized)")
print("=" * 60)
...
state_path = MODEL_INT4 / "quantized_state.pt"
...
async def root():
    return {
        "message": "OmniVoice TTS Server (INT4 Quantized)",
        "docs": "/docs",
        "voices": "/v1/audio/voices",
        "optimization": "INT4 weight-only (TorchAO)",
    }


@app.get("/health")
async def health():
    return {"status": "ok", "model": "int4_quantized", "voices": len(voices)}
...
async def docs():
    return {
        ...
        "optimization": "INT4 weight-only (TorchAO, group_size=128)",
...
if __name__ == "__main__":
    ...
    print(f"Quantization:       INT4 weight-only (group_size=128)")
    print(f"Expected VRAM:      ~2.2 GB (vs 3.5 GB INT8, 6 GB BF16)")
    print(f"Expected disk:      ~2.0 GB (vs 2.24 GB INT8, 3.81 GB BF16)")
    print("=" * 60 + "\n")

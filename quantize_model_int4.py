"""
Quantize OmniVoice BF16 model to INT4 weight-only using TorchAO.

This script:
1. Loads the BF16 model from OmniVoice/
2. Applies INT4 weight-only quantization (group_size=128) to all Linear layers
3. Saves the quantized state dict to OmniVoice_INT4/
4. Copies config, tokenizer, and audio_tokenizer files

Usage:
    python quantize_model_int4.py              # Uses OmniVoice/ as source
    python quantize_model_int4.py --no-save    # Quantize and test without saving
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Quantize OmniVoice to INT4 weight-only"
    )
    parser.add_argument(
        "--source",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="OmniVoice_INT4",
        help="Output INT4 model directory (default: OmniVoice_INT4)",
    )
    parser.add_argument(
        "--no-save",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Group size for INT4 quantization (default: 128). Lower = better quality, larger size.",
    )
    ...
    print("=" * 60)
    print("OmniVoice INT4 Weight-Only Quantization")
    print("=" * 60)
    print(f"Source:  {source_dir}")
    print(f"Output:  {output_dir}")
    print(f"Method:  TorchAO Int4WeightOnlyConfig (group_size={args.group_size})")
    ...
    print(f"\n[2/4] Applying INT4 weight-only quantization...")
    print(f"     Quantizing LLM backbone (197 Linear layers)...")
    t0 = time.time()

    quantize_(model.llm, Int4WeightOnlyConfig(group_size=args.group_size))

    print(f"     Quantizing audio head (1 Linear layer)...")
    quantize_(model.audio_heads, Int4WeightOnlyConfig(group_size=args.group_size))
    ...
    for name, module in model.named_modules():
        ...
        if "Int4WeightOnly" in type(module).__name__:
            quantized_count += 1
    ...
    try:
        audio = model.generate(
            text="Hello, this is a test of INT4 quantized OmniVoice with group size "
            + str(args.group_size)
            + ".",
            num_step=16,
            speed=1.0,
    ...
    if config_path.exists():
        with open(config_path, "r") as f:
            config = json.load(f)
        config["quantization"] = {
            "method": "torchao.Int4WeightOnlyConfig",
    ...
    print(
        f"Output INT4 size:  {sum(f.stat().st_size for f in output_dir.rglob('*') if f.is_file()) / (1024**3):.2f} GB"
    )
    print(f"Expected runtime:  ~2.2 GB (vs ~3.5 GB INT8, ~6 GB BF16)")
    print(f"Model path:        {output_dir}")
    print(f"{'=' * 60}")
    print(f"\nTo use this model, run:")
    print(f"  start_int4.bat        (Gradio web UI)")
    print(f"  start_api_int4.bat    (REST API server)")

"""
OmniVoice INT8 Quantized Gradio Server (XPU + TorchAO + torch.compile)

Mirrors server.py but loads the INT8 quantized model from OmniVoice_INT8/
...
def apply_compile(model):
    print("Skipping torch.compile() (too slow on Windows XPU)")
    return model
...
with gr.Blocks(title="OmniVoice TTS Server (INT8)") as demo:
    gr.Markdown("# OmniVoice TTS Server (INT8 Quantized + XPU)")
